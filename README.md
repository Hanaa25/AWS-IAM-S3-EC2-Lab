...
The `S3-Support` group permissions allow `user-1` to access Amazon S3 resources.
...
`user-1` can access Amazon S3 resources.
...
`user-2` can access Amazon EC2 resources.
...
- IAM Groups can be used to manage permissions for users.
- IAM policies define which AWS resources and actions are allowed.
